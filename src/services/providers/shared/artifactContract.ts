    ...
    return value
        .map((entry, index): ArtifactEvidence | null => {
            ...
            return {
                ...
                sourceTitle:
                    toDisplayText(record.sourceTitle ?? record.source_name ?? record.source).trim() || undefined,
                sourceUrl:
                    toDisplayText(record.sourceUrl ?? record.url ?? record.uri).trim() || undefined,
...
export const buildArtifactFromPayload = (
    ...
    const evidence = [...payloadEvidence, ...(options.extraEvidence || [])]
