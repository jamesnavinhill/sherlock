...
export const normalizeSource = (source: {
  ...
  const rawUrl =
    typeof source.url === 'string' ? source.url : typeof source.uri === 'string' ? source.uri : '';
...
export const normalizeFeedItems = (
  ...
  return items.map((item, index) => {
    ...
    const rawRiskLevel = normalizeHumanText(
    ...
    return {
      id:
        normalizeHumanText(getCaseInsensitiveField(record, 'id')) ||
...
export const normalizeLiveEvents = (value: unknown, idPrefix: string): MonitorEvent[] => {
  ...
  return items.map((item, index) => {
    ...
    const type =
      rawType === 'SOCIAL' || rawType === 'NEWS' || rawType === 'OFFICIAL' ? rawType : 'NEWS';

    const rawSentiment = normalizeHumanText(
      getCaseInsensitiveField(record, 'sentiment')
    ).toUpperCase();
    const sentiment =
      rawSentiment === 'NEGATIVE' || rawSentiment === 'NEUTRAL' || rawSentiment === 'POSITIVE'
        ? rawSentiment
        : 'NEUTRAL';
    ...
    const threatLevel =
      rawThreatLevel === 'INFO' || rawThreatLevel === 'CAUTION' || rawThreatLevel === 'CRITICAL'
        ? rawThreatLevel
        : 'INFO';

    return {
      id: normalizeHumanText(
        getCaseInsensitiveField(record, 'id') ?? getCaseInsensitiveField(record, 'eventId'),
