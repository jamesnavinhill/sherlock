...
export const BackgroundMatrixRain = () => {
  ...
  useEffect(() => {
    ...
    const draw = (timestamp: number) => {
      ...
      // Softer trail in light mode to avoid muddy overlays.
      ctx.fillStyle = isLightMode ? 'rgba(248, 242, 230, 0.18)' : 'rgba(0, 0, 0, 0.05)';
      ...
      for (let i = 0; i < drops.length; i++) {
        ...
        // Sparse rain
        if (Math.random() > 0.1) {
          ctx.globalAlpha = isLightMode ? 0.08 : 0.15;
  ...
  return (
    <canvas
      ref={canvasRef}
      className="matrix-rain-canvas absolute inset-0 pointer-events-none opacity-50 z-0"
    />
  );
};
