...
import {
    ...
    ArrowRight, Clock, X, Command, Hash
} from 'lucide-react';
import { AppView, InvestigationReport, Case, Headline } from '../../types';

export const GlobalSearch: React.FC = () => {
    const {
        archives, cases, headlines, setCurrentView,
        showGlobalSearch, setShowGlobalSearch, setActiveTaskId, tasks,
        addTask
    } = useCaseStore() as any;
    ...
    const results = useMemo(() => {
        ...
        uniqueEntities.forEach(e => {
            output.push({ type: 'ENTITY', title: e, icon: User });
        });
    ...
    const handleSelectResult = (result: any) => {
        if (result.type === 'REPORT') {
            ...
            const existingTask = tasks.find((t: any) => t.report?.id === report.id);
            if (existingTask) {
                        ...
                        <div className="p-2 space-y-1">
                            {results.map((result, index) => (
                                <button
                                    ...
                                    className={`w-full flex items-center p-3 font-mono text-left transition-colors ${index === selectedIndex ? 'bg-osint-primary/10 border border-osint-primary/30' : 'border border-transparent hover:bg-zinc-900'
                                        }`}
                                >
                                    <div className={`w-8 h-8 rounded flex items-center justify-center mr-4 ${index === selectedIndex ? 'text-osint-primary' : 'text-zinc-600'
                                        }`}>
                                        <result.icon size={18} />
                                    </div>
                                    ...
                                    <ArrowRight className={`ml-4 w-4 h-4 transition-transform ${index === selectedIndex ? 'translate-x-0 opacity-100 text-osint-primary' : '-translate-x-2 opacity-0'
                                        }`} />
                                </button>
                            ))}
                        </div>
