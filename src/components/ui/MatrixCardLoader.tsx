...
export const MatrixCardLoader = ({ active }: { active: boolean }) => {
    ...
    return (
        <div ref={containerRef} className="h-full bg-black border border-zinc-800 relative overflow-hidden flex items-center justify-center group min-h-[14rem]">
            {active && <canvas ref={canvasRef} className="absolute inset-0 opacity-30" />}
            ...
            <div className="relative z-10 flex flex-col items-center p-3 bg-black/60 backdrop-blur-sm border border-zinc-700/50">
                <div className="flex space-x-1.5 mb-2">
                    <div className={`w-1.5 h-1.5 bg-osint-primary ${active ? 'animate-pulse' : ''}`} style={{ animationDelay: '0ms' }}></div>
                    <div className={`w-1.5 h-1.5 bg-osint-primary ${active ? 'animate-pulse' : ''}`} style={{ animationDelay: '300ms' }}></div>
                    <div className={`w-1.5 h-1.5 bg-osint-primary ${active ? 'animate-pulse' : ''}`} style={{ animationDelay: '600ms' }}></div>
                </div>
