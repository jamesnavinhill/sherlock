  ...
  return (
    <nav className="flex items-center space-x-1 text-xs font-mono overflow-x-auto max-w-full min-w-0">
      {items.map((item, index) => {
        const displayLabel = sanitizeDisplayTitle(item.label);
        ...
        return (
          <React.Fragment key={item.id}>
            ...
            >
              {item.type === 'CASE' && (
                <FolderOpen className="w-3 h-3 text-osint-primary flex-shrink-0" />
              )}
              {item.type === 'REPORT' && (
                <FileText className="w-3 h-3 text-zinc-500 flex-shrink-0" />
              )}
              <span className="uppercase tracking-wide truncate">{displayLabel}</span>
            </button>
          </React.Fragment>
        );
      })}
