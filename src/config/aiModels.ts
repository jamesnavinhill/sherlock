...
export const getOpenRouterQuickPicks = (): AIModelOption[] => {
    ...
    const quickPicks = OPENROUTER_QUICK_PICK_IDS.map((id) => catalog.find((model) => model.id === id)).filter(
        (model): model is AIModelOption => !!model
    );
...
export const getRecentModelSelections = (provider?: AIProvider): AIModelOption[] => {
    const catalog = provider ? getRuntimeReadyModelsForProvider(provider) : [...STATIC_MODELS, ...getOpenRouterModels()];
...
export const getModelOptionById = (modelId: string): AIModelOption | undefined => {
    const knownModel = [...STATIC_MODELS, ...getOpenRouterModels()].find((model) => model.id === modelId);
...
export const getTemplateSelectableModels = (selectedModelId?: string): AIModelOption[] => {
    const openRouterCompact = getCompactModelChoicesForProvider('OPENROUTER', selectedModelId).slice(0, 4);
    const directProviderModels = STATIC_MODELS.filter((model) => model.capabilities.runtimeStatus === 'ACTIVE');
...
export const refreshOpenRouterModelCatalog = async (
    ...
    const models = dedupeModels((payload.data || []).map(normalizeOpenRouterModel).filter((model): model is AIModelOption => !!model));
