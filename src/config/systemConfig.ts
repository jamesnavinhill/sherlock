...
export const migrateSystemConfig = (value?: Partial<SystemConfig> | null): SystemConfig => {
    ...
    return {
        ...
        openRouter: {
            ...
            ...(raw.openRouter || {}),
            webSearchEnabled:
                raw.openRouter?.webSearchEnabled !== false,
            engine:
            ...
            searchContextSize:
                raw.openRouter?.searchContextSize === 'low' ||
                raw.openRouter?.searchContextSize === 'high'
                    ? raw.openRouter.searchContextSize
                    : 'medium',
            allowedDomains: Array.isArray(raw.openRouter?.allowedDomains)
                ? raw.openRouter.allowedDomains.filter((entry): entry is string => typeof entry === 'string')
                : [],
            excludedDomains: Array.isArray(raw.openRouter?.excludedDomains)
                ? raw.openRouter.excludedDomains.filter((entry): entry is string => typeof entry === 'string')
                : [],
        },
    };
};
